...
export default function CaseTimelineSteps({ caseData }) {
  ...
  return (
    <div>
      ...
      <div className="position-relative" style={{ marginLeft: 24 }}>
        ...
        <ol className="timeline ps-0 position-relative" style={{ paddingLeft: 0 }}>
          {caseData.timeline.map((item, idx) => (
            <li key={idx}
                className={`timeline-item position-relative mb-5 d-flex ${visibleStep > idx ? 'animate-slide-in' : ''}`}
                style={{ minHeight: 80 }}>
              <div style={{
                position: "absolute",
                left: "-24px",
              ...
              <div className="ms-5 flex-grow-1">
                ...
                {item.amount && <div className="mb-1">
                  <span className="badge bg-white border border-success text-success"
                        style={{ fontWeight: 600, letterSpacing: 1, fontSize: 16 }}>{item.amount}</span>
                </div>}
                {item.proof && <div className="mb-2">
                  <a href={item.proofUrl} className="btn btn-outline-success btn-sm px-3" target="_blank" rel="noopener noreferrer">View Proof</a>
                </div>}
              </div>
            </li>
          ))}
      ...
      <style>{`
        .timeline-item { opacity: 0; transform: translateY(30px); transition: all 0.6s cubic-bezier(0.4,0,0.2,1);}
        .timeline-item.animate-slide-in { opacity: 1; transform: none; }
      `}</style>
    </div>
  );
}
